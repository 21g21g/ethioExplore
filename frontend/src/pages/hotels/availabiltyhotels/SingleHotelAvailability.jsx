...
const SingleHotelAvailability = () => {
  ...
  const handleClick = async () => {
    if (selectedRooms.length > options.room) {
      toast.error(`you cannot select more than ${options.room} rooms`)
    }
    else {
      try {
  ...
  return (
    ...
    <div className='flex flex-col'>
        ...
        <div className="flex flex-row justify-between">
          <div className="flex flex-col md:flex-row justify-between">
            <div className="flex flex-col">
              ...
              <p className="text-emerald-500">Book a room for ${singleHotelData.cheapestPrice} and enjoy yourself.</p>
            </div>
          </div>
        </div>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:self-center flex-wrap mt-4">
          {singleHotelData.photos.map((photo, index) => (
            <img key={index} src={`http://localhost:5000/${photo}`} className="w-full md:w-48 h-40 object-cover rounded-md" alt={`Photo ${index + 1}`} />
          ))}
        </div>
          ...
          <div className="flex flex-col">
            ...
            <div className="flex flex-col p-4 bg-gray-300 w-full md:w-1/4 rounded-md">
              <p>This room is reserved for {getDateDifference(new Date(dates[0].endDate), new Date(dates[0].startDate))} days.</p>
              ...
              <div>
                <button onClick={handleModal} className="text-white mt-2 bg-green-500 hover:bg-green-600 px-4 py-2 rounded-md w-full">Reserve Now</button>
              </div>
            </div>
        </div>
      </div>
      ...
      <Modal show={modal} onClose={() => setModal(false)}>
        <ModalHeader>
          <h1 className='text-3xl text-center text-custom-green2'>Select the room you want to reserve:</h1>
        </ModalHeader>
        <ModalBody>
          <div className='flex flex-col'>
            {roomData.map((room) => (
